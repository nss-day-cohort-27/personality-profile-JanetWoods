...
//     const myString = (localStorage.getItem(mydat))

// //     let myDat = (JSON.parse(myString))
// // // }
...
let generalMe = document.getElementsByClassName("generalprofile")[0]
...
const databaseString = localStorage.getItem("myRawData")
...
// const stringified = JSON.stringify(littleDebbieProducts)
// localStorage.setItem("littleDebbieProducts", stringified)



let write = document.querySelector(".generalStuff")


// function writeToDom(data){
for (let index = 0; index < data.general.length; index++) {
    const element = data.general[index];
    write.innerHTML = `<p>${element.description}</p>`
}
    // console.log(data)
// }
// writeToDom(data)
